...
Then(/^the "([^"]*)" should( not)? equal the "([^"]*)" stored in global variables$/,
    async function (this: ScenarioWorld, elementKey: ElementKey, negate:boolean, variableName: string) {
        ...
        console.log(`Verifying that the element "${elementKey}" should${negate ? " not" : ""} equal the value stored in global variable "${variableName}"`);
    ...
    async function (this: ScenarioWorld, elementKey: ElementKey, negate:boolean, variableName: string) {
        ...
        console.log(`Verifying that the element "${elementKey}" should${negate ? " not" : ""} contain the value stored in global variable "${variableName}"`);
        const elementIdentifier = getElementLocator(page, elementKey, globalConfig);
        await waitFor(async () => {
            const elementText = await page.textContent(elementIdentifier);
            const expectedValue = globalVariables[variableName];
